...
export const Currency = ({ data }) => {
	const sortedKeys = Object.keys(data.Valute).sort((a, b) => {
		const currA =
			(data.Valute[a].Value - data.Valute[a].Previous) / data.Valute[a].Nominal
		const currB =
			(data.Valute[b].Value - data.Valute[b].Previous) / data.Valute[b].Nominal
		return currB - currA
	})
	...
	return (
		<div className={styles.page}>
			...
			<div className={styles.items}>
				{sortedKeys.map(key => {
					const curr =
						(data.Valute[key].Value - data.Valute[key].Previous) /
					...
					return (
						<div key={key} className={styles.item}>
							<div className={styles.info}>
								...
								<p>
									{(data.Valute[key].Value / data.Valute[key].Nominal)
										.toString()
										.slice(0, 6)}
